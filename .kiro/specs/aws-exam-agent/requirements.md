...
4. WHEN 問題を生成する THEN システム SHALL 読み込んだ試験ガイドに基づいて以下の情報を自動判定・出力する：
   ...
   - **ガイド参照**: 試験ガイドの具体的な項目参照（形式は試験により異なる）
...
- **追加フィールド**:
  ...
  - `guide_reference`: 試験ガイドの項目参照（汎用的な命名）
...
- **段階的実装**:
  - **Phase 1**: シンプルなファイル読み込み + プロンプト統合
    - 狙い: 最小限の実装で機能検証
    - 方法: 試験ガイド全体をシステムプロンプトに含める
    - 課題: トークン制限に引っかかる可能性
  - **Phase 2**: 必要に応じて圧縮機能追加
    - 狙い: トークン制限対応とコンテキスト最適化
    - 方法: LLMLingua 等による試験ガイド圧縮
    - 効果: コンテキストサイズを約 1/3 に削減
  - **Phase 3**: 動的コンテキスト選択で最適化
    - 狙い: トークン効率と問題品質の両立
    - 方法: カテゴリに基づく関連セクションの動的抽出
    - 効果: 必要な情報のみでコンテキストサイズを最小化
- **実装判断基準**:
  - Phase 1 → Phase 2: トークン制限エラーが発生した場合
  - Phase 2 → Phase 3: より精密なコンテキスト制御が必要な場合
...
**対象**: 現在組織展開中のサービス  
**最終更新**: 2025 年 1 月 6 日（試験ガイド活用による問題分類表示機能要件追加・汎用性向上）
