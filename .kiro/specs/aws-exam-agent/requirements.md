...
### Requirement 6: ジャンル分散機能（問題生成の偏り解消）

**User Story:** As a 組織メンバー, I want 毎日異なるジャンル・タスクから問題が生成される機能, so that 試験ガイドの全領域をバランス良く学習でき、特定分野への偏りを避けることができる

#### Acceptance Criteria

1. WHEN 問題生成を開始する THEN システム SHALL 過去の問題生成履歴を参照して最近使用されたタスクを特定する
2. WHEN タスク選択を実行する THEN システム SHALL 試験ガイドで定義された全タスクから最近使用されていないタスクを優先選択する
3. WHEN タスク履歴が存在しない場合 THEN システム SHALL 試験ガイドで定義された分野別重み設定に基づいてランダム選択する
4. WHEN 同一タスクが連続する場合 THEN システム SHALL 強制的に別のタスクを選択する（連続回避機能）
5. WHEN 特定タスクの使用頻度が高い場合 THEN システム SHALL そのタスクの選択確率を一時的に下げる（頻度調整機能）
6. WHEN 問題生成プロンプトを作成する THEN システム SHALL 選択されたタスクに関連する試験ガイドセクションを重点的に参照するよう指示する
7. WHEN 問題生成が完了する THEN システム SHALL 使用したタスク情報を履歴として記録する
8. WHEN 履歴を記録する THEN システム SHALL タスク ID、生成日時、問題内容の概要を保存する

#### 技術的実装要件

**タスク管理システム**:

- **タスク定義**: 試験ガイドで定義されたタスク構造を動的に解析・管理
- **重み設定**: 試験ガイドで定義された分野別重み設定を動的に反映
- **汎用性**: 異なる試験（AWS、Azure、GCP、OCI 等）のガイド構造に対応
- **履歴管理**: 過去 N 日間（推奨 7-14 日）の使用タスク履歴を保持

**選択アルゴリズム**:

- **優先度計算**: 最終使用日からの経過日数 × 分野重み × ランダム要素
- **連続回避**: 直前のタスクと同一タスクの選択を禁止
- **頻度調整**: 使用頻度の高いタスクの選択確率を動的に調整
- **フォールバック**: アルゴリズム失敗時の安全な選択機能

**プロンプト最適化**:

- **タスク特化**: 選択されたタスクに関連するガイドセクションを重点参照
- **コンテキスト絞り込み**: 関連性の低いセクションの重要度を下げる
- **品質維持**: タスク特化による問題品質の向上

**データ永続化**:

- **履歴保存**: タスク使用履歴の永続的な保存機能
- **パフォーマンス**: 高速な履歴参照・更新機能
- **メンテナンス**: 古い履歴の自動削除機能

**検証・監視**:

- **分散確認**: 生成された問題のタスク分布を可視化
- **効果測定**: 偏り解消効果の定量的評価
- **調整機能**: アルゴリズムパラメータの動的調整

### Requirement 7: 学習戦略支援機能の強化
...
### Requirement 8: 問題品質評価・改善機能

**User Story:** As a システム管理者, I want 生成される問題の品質を定量的に評価・改善する機能, so that より実用的で試験に近い高品質な問題を継続的に提供できる

#### Acceptance Criteria

1. WHEN 問題を生成する THEN システム SHALL 各問題に対して品質スコアを自動算出する
2. WHEN 品質スコアを算出する THEN システム SHALL 以下の評価軸で総合的に評価する：
   - **技術的正確性**: AWS 公式ドキュメントとの整合性（0-100 点）
   - **難易度適合性**: Professional レベルの複雑度評価（0-100 点）
   - **構造的品質**: 問題文・選択肢・解説の構造評価（0-100 点）
   - **試験ガイド準拠**: 出題傾向・分野バランスとの一致度（0-100 点）
3. WHEN 品質スコアが閾値を下回る THEN システム SHALL 自動的に問題を再生成する
4. WHEN 問題品質を評価する THEN システム SHALL AWS 公式サンプル問題を参考基準として活用する
5. WHEN 品質データを蓄積する THEN システム SHALL 品質傾向の分析・可視化機能を提供する
6. WHEN 品質改善を実行する THEN システム SHALL 低品質要因の特定と改善提案を自動生成する

#### 技術的実装要件

**品質評価システム（MVP）**:

- **評価ツール**: `evaluate_question_quality` カスタムツール実装
- **評価軸**: 4 軸評価（技術的正確性・難易度・構造・準拠性）
- **評価基準**: 試験ガイド + 公式サンプル問題による品質基準
- **スコア算出**: 重み付き総合スコア（0-100 点）
- **エージェント統合**: Strands Agents による自律的品質判定

**品質改善システム（MVP）**:

- **改善ツール**: `improve_question_quality` カスタムツール実装
- **自動改善**: 品質フィードバックに基づく問題改善
- **改善サイクル**: 品質基準達成まで最大 3 回改善
- **無限ループ防止**: 改善回数制限による安全な処理

**汎用性確保**:

- **モデル非依存**: 任意の LLM で動作可能
- **プロバイダー非依存**: 異なるクラウドプロバイダーに対応
- **試験タイプ非依存**: 様々な試験形式に対応可能
...
**対象**: 現在組織展開中のサービス  
**最終更新**: 2025 年 9 月 21 日（ジャンル分散機能追加・問題生成偏り解消対応）
