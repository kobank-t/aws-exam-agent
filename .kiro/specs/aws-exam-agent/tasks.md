...
- [ ] 7. 基本実装: シンプルなファイル読み込み + プロンプト統合
  ...
  **サブタスク**:
  ...
  - [ ] 7.5 分類情報生成のプロンプト指示追加・テスト実行

  _Requirements: Requirement 5 (試験ガイド活用による問題分類表示機能) - 基本実装_

- [ ] 8. AgentInput.exam_type デフォルト値変更と汎用性向上

  **完了基準**:

  - `AgentInput.exam_type` デフォルト値を "AWS-SAP-C02" に変更完了
  ...
  **サブタスク**:
  ...
  - [ ] 8.1 AgentInput.exam_type デフォルト値を "AWS-SAP-C02" に変更
  - [ ] 8.2 EXAM_TYPES 辞書をファイル名ベースに更新
  - [ ] 8.3 既存機能の動作確認・後方互換性テスト
  - [ ] 8.4 Teams 投稿での学習分野情報表示実装
  - [ ] 8.5 汎用性確保のための最終調整・品質チェック
