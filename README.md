...
`.env` ファイルで Bedrock モデルとリージョンを設定：

```bash
# Bedrock モデルID（デフォルト: us.anthropic.claude-sonnet-4-5-20250929-v1:0）
BEDROCK_MODEL_ID=us.anthropic.claude-sonnet-4-5-20250929-v1:0

# Bedrock リージョン（デフォルト: us-east-1）
BEDROCK_REGION=us-east-1
```

**主要なモデル ID 例**:

- `us.anthropic.claude-sonnet-4-5-20250929-v1:0`: **Claude Sonnet 4.5（US Cross-Region）** ✅ 推奨
...
> **💡 推奨設定**: US Cross-Region Inference Profile は AgentCore CLI が生成する IAM ポリシーで動作します。JP/Global Profile を使用する場合は追加の IAM 権限設定が必要です。
...
- **AI 基盤**: Amazon Bedrock (Claude Sonnet 4.5) + AgentCore + Strands Agents + MCP 統合
